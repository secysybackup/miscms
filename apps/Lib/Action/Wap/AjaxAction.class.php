...
class AjaxAction extends PublicAction
{
    ...
    public function address()
    {
        ...
        if ($do=='save') {
            ...
            $_POST['isdefault']=1;
            if ($userid) {
                $model_db->where("userid=".$userid)->save(array('isdefault'=>0));
                if($id){
                  $r = $model_db->save($_POST);
                  if($model_db->getDbError())die(json_encode(array('id'=>0)));
                  $_POST['edit'] =1;
                }else{
                    ...
                    $ir = $model_db->where($where)->find();
                    if($ir){
                        echo json_encode(array('error'=>'收货信息已经存在！'));exit;
                    }
                    $id=$model_db->add ($_POST);
                }
            } else {
                $_POST['id']=1;
