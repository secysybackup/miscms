...
class RBAC extends Think
{
    ...
    static public function getAccessList($authId)
    {
        ...
        $sql    =   "select node.id,node.name from ".
                ...
                $table['node']." as node ".
                "where user.id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=1 and node.pid={$appId} and node.status=1";
        $modules =   $db->query($sql);$_SESSION['modules'] = $modules;
        // 判断是否存在公共模块的权限
        $publicAction  = array();
        foreach($modules as $key=>$module) {
            ...
            $sql    =   "select node.id,node.name from ".
                ...
                $table['access']." as access ,".
                $table['node']." as node ".
                "where user.id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$moduleId} and node.status=1";
                $rs =   $db->query($sql);
                foreach ($rs as $a){
