...
class  ThinkTemplate {
    ...
    public function parse($content) {
        ...
        //解析普通模板标签 {tagName}
        $content = preg_replace_callback('/('.$this->config['tmpl_begin'].')([^\d\s'.$this->config['tmpl_begin'].$this->config['tmpl_end'].'].+?)('.$this->config['tmpl_end'].')/is',function($matches) use($that){return $that->parseTag($matches[2]);},$content);
        return $content;
    }
    ...
    protected function parseExtend($content) {
        ...
        if($find) {
            ...
            preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is',function($matches) use($that) {return $that->parseBlock($matches[1],$matches[2]);},$content);
            // 读取继承模板
            $array      =   $this->parseXmlAttrs($matches[1]);
            ...
            $content    =   preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is',function($matches) use($that) {return $that->replaceBlock($matches[1],$matches[2]);},$content);
        }else{
    ...
    public function parseTag($tagStr){
        ...
        // yuzihao add 添加if判断
        }elseif(substr($tagStr,0,3)=='if '){  //yuzihao add
            ...
            return '<?php '.substr($tagStr,4).';?>';  //yuzihao add end
